...
public class UserPortTo24BitAddress extends Device {
    ...
    public int getCIA2InterruptControl() {
        return CIA2InterruptControl;
    }

    public void setAssertNMI(boolean assertNMI) {
        this.assertNMI = assertNMI;
    }

    boolean assertNMI = false;

    int CIA2InterruptControl = 0;
    @Override
    public void write(int address, int data) throws MemoryAccessException {
        ...
        switch (register) {
            ...
            case 0x10d:
                if ((data & 0x80) == 0) {
                    // Clear bits
                    CIA2InterruptControl = CIA2InterruptControl & ~(data & 0x7f);
                } else {
                    // Set bits
                    CIA2InterruptControl = CIA2InterruptControl | (data & 0x7f);
                }
...
//                                    return 0xff;
            ...
            case 0x10d:
                ...
                if ((CIA2InterruptControl & 0x10) == 0x10) {
                    // Then we have NMIs generated by the video externally, the new way of handling VBLANK flags
                    if (logRead) {
                        assertNMI = false;
                    }
                } else {
                    // Old way of supporting VBLANK flags
                    for (MemoryBus device : externalDevices) {
