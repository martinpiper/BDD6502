...
public class AudioExpansion3 extends MemoryBus implements Runnable {
    int addressRegisters = 0x8000, addressExRegisters = 0x01;
    int addressExSampleBank = 0x06;

    public static final int sampleRate = 74626 / 2;// Divided by 2 from real hardware 74626 Hz to reduce CPU, note the counterShift and counterShiftValue test differs to the hardware to compensate
    ...
    public static int calculateRateFromFrequency(int frequency) {
        return (AudioExpansion3.counterShiftValue * frequency) / AudioExpansion3.sampleRate;
    }

    public AudioExpansion3() {
    }

    int channel = 0;
    public AudioExpansion3(int addressRegisters, int addressExSampleBank, int channel) {
    ...
    public void start() {
        ...
        try {
            outSamples = new BufferedOutputStream(new FileOutputStream("target/debugchannel3.pcmu8"));
